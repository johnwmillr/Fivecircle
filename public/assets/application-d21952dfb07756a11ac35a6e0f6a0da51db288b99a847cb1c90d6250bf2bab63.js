...
(function() {
  jQuery(function() {
    var coords, fileUploadAJax, geo_error, geo_options, map, map_export, marker, markersArray, updateMap, zoom_and_pan_were_set;
    markersArray = [];
    map = null;
    map_export = null;
    marker = null;
    ...
    updateMap = function(location) {
      ...
      return marker.setPosition(new google.maps.LatLng(coords.latitude, coords.longitude));
    };
    fileUploadAJax = function(location) {
      var lat, lon;
      console.log(coords);
      lat = coords.latitude;
      lon = coords.longitude;
      console.log(lat);
      return $.ajax('save_location', {
        type: "POST",
        dataType: "text",
        data: {
          coordinates: {
    ...
    return $(document).ready(function() {
      $(document).on("click", "#medium_image", function() {
        return fileUploadAJax();
      });
      return setInterval(function() {
        return navigator.geolocation.getCurrentPosition(updateMap, geo_error, geo_options);
      }, 1000);
    });
  });
...
window.onload = function() {
    ...
    for (var i = 0, row; row = table.rows[i]; i++) {
       ...
       alert('add marker at (' + lat + ', ' + lon + ') with icon ' + url + ' to map ' + map_export);
